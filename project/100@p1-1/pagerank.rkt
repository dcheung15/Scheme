...
(define (num-pages graph)
  (define i (length (remove-duplicates (flatten graph)))) ;;makes list of pairs into normal list, removes duplicates in list and gets length of list
  i) ;;returns the length of the list
...
(define (num-links graph page)
  ;; Checks each pair if there are pairs share the same page
  (define (loop graph l) ;;from slide 7 of the "The Page Rank Algorithm Slides"
    (match graph
      [`() l]
      [`((,p0 ,p1) . ,rst)
       (if (equal? p0 page) (loop rst (cons p1 l)) (loop rst l))]))
  (define listLength (length (loop graph '()))) ;;gets the length the loop
...
(define (get-backlinks graph page)
  (list->set (map first (filter (lambda (x) (equal? (second x) page)) graph)))
  )
...
(define (mk-initial-pagerank graph)
  (define (num-pagesList graph) ;;helper for grabbing the list of pages w/ no duplicates
  (define x (remove-duplicates (flatten graph)))
    x) ;;removes duplicates in list
  (define v (/ 1 (length (num-pagesList graph)))) ;;takes 1 divided by the # of pages in the list
  (define (h lst acc) ;; creates the hash
    (match lst
      ['() acc]
      [`(,hd . ,tl) (h tl (hash-set acc hd v))]))
  (h (append* graph) (hash))
  )
...
(define (step-pagerank pr d graph)
  (define (sumpage graph pr sec)
    ;; Fold 0 to map across the 
    (foldl + 0 (map (lambda (x) (/ (hash-ref pr x) (num-links graph x)))
                    (set->list (get-backlinks graph sec)))))
  (define (helper lst pr d graph)
    (define (loop l)
      (match l
        ['() (hash)]
        [`(,p0 . ,p1) 
         ;; Loop through the rest of the list and NextPageRank(page-i) = (1 - d) / N + d * S
           (hash-set (loop p1) p0 (+ (* (sumpage graph pr p0) d) (/ (- 1 d) (num-pages graph))))]))
    (loop lst))
  (helper (append* graph) pr d graph))
